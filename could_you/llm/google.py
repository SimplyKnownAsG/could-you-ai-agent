...
class GoogleLLM(BaseLLM):
    ...
    async def _call_client(self):
        try:
            ...
            msg = f"Error while calling Google AI Developer API: {err}"
            LOGGER.error(msg)
    ...
    def _model_name(self) -> str:
        ...
        if not model:
            raise CYError(
                message="Google provider requires llm.args.model",
                retriable=False,
    ...
    def _transform_response(self, response) -> Message:
        ...
        if not content:
            raise CYError(
                message=f"Cannot handle Google AI Developer API response, sry... {response}",
                retriable=False,
    ...
    def _sanitize_schema(self, value: T) -> T:
        ...
        such as `_def`, `~standard`, `$schema`, `additional_properties`, and
        other descriptive fields. The Gemini Developer API rejects some of these
        schema fields, so strip obvious decoration recursively.
        """
